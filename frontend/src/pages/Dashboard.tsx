...
export function Dashboard() {
  ...
  return <section><header className="page-head"><div><p className="eyebrow">ศูนย์ตรวจสอบหลักฐานภาพ</p><h1>ภาพรวมระบบ</h1><p>ข้อมูลจะอัปเดตจากผลประมวลผลล่าสุดอัตโนมัติทุก 10 วินาที</p></div><div className="live"><span />LIVE</div></header>
    ...
    <div className="panel"><div className="panel-head"><div><h2>ลำดับการตรวจสอบ</h2><p>ระบบคัดกรองภาพอย่างรวดเร็วก่อน แล้วตรวจเฉพาะคู่ที่น่าสงสัยอย่างละเอียด</p></div></div><div className="pipeline">{["ตรวจไฟล์ที่เหมือนกัน", "จดจำลักษณะสำคัญของภาพ", "ค้นหาคู่ภาพที่น่าสงสัย", "ตรวจรายละเอียดและตำแหน่งที่ตรงกัน", "สรุปผลการใช้ภาพซ้ำ"].map((item, i) => <div key={item}><span>{String(i + 1).padStart(2, "0")}</span><strong>{item}</strong></div>)}</div></div>
  </section>;
}
