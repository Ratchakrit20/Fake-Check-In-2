...
รับไฟล์และตรวจความปลอดภัย
        |
        v
SHA-256 ตรวจไฟล์ตรงกันทุกไบต์ + เก็บ pHash มุมปกติ
        |
        v
SSCD สร้างลายนิ้วมือสำหรับตรวจสำเนาเป็น batch (GPU เมื่อใช้ได้)
        |
        v
ค้น Candidate ขนานกัน
├─ FAISS เลือก embedding ใกล้เคียง Top-K
├─ pHash query 0/90/180/270 องศา ค้น BK-tree
└─ SHA-256 เก็บไฟล์ซ้ำตรงตัว
        |
        v
รวม Candidate -> ตัดคู่ซ้ำ/ภาพเดียวกัน/source เดียวกัน
        |
        v
...
- pHash BK-tree สร้างใน RAM หนึ่งต้นต่อหนึ่งงานจาก pHash มุมปกติของภาพทั้งหมด แล้วใช้ query สี่มุมค้น `image_id` ใกล้เคียง
...
│  │  └─ phash_index.py           BK-tree สำหรับค้น pHash ใกล้เคียงโดย Hamming distance
│  ├─ services/
...
เข้าเมนู “วิเคราะห์หลายภาพ” เลือกหลายไฟล์หรือโฟลเดอร์ แล้วเริ่มวิเคราะห์ สามารถเลือกได้สูงสุด 5,000 รูปต่อครั้ง รูปละไม่เกิน 5 MB และรวมไม่เกิน 1,000 MB หน้าเว็บยังส่งทั้งหมดเป็น request เดียว ส่วน Backend แบ่ง embedding และการตรวจคู่เป็นชุดภายใน สามารถเปลี่ยนหน้า/รีเฟรชได้ งานไม่เริ่มใหม่และหน้าเว็บจะเชื่อมกลับไปยังงานเดิม
...
- คู่ที่มีเลข 10 หลักหลัง `home_`/`splitter_` ตรงกันจะไม่ส่งเข้า SIFT เพราะชื่อระบุ source เดียวกันอยู่แล้ว
- ภาพหมุนใช้ pHash query สี่มุมค้น BK-tree ใน RAM โดยไม่วนเทียบ pHash กับทุกภาพ แล้วรวมผลกับ FAISS และ SHA-256 ก่อนส่งเข้า SIFT/RANSAC
...
สำหรับหลายพันภาพไม่ควรเปรียบเทียบทุกคู่ ระบบจึงใช้ FAISS Top-K, pHash BK-tree, SHA-256 และตัวกรอง SSCD ก่อนส่งเข้า detector ที่หนักกว่า
